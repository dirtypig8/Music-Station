...
- 🔊 **主機播放** — 音樂在伺服器主機的喇叭播放
- 🎧 **瀏覽器收聽** — 連上網頁的人也能直接在自己的裝置聽到音樂（手機、電腦皆可）
- 📋 **歌曲佇列** — 自動排隊、自動播下一首
...
連上網頁的人可以直接在自己的手機或電腦上聽到音樂：

1. 進入網頁後，在「正在播放」區塊找到 **🎧 點擊收聽** 按鈕
2. 點擊後即開始在瀏覽器播放音樂
3. 播放進度會自動與伺服器同步
4. 可獨立調整瀏覽器端的音量（不影響主機音量）

> 📱 **iOS 使用者注意**：iPhone/iPad 不支援網頁調整音量，請使用手機實體音量鍵。
...
### Q: iPhone 上收聽音量滑桿沒反應
這是 iOS 系統限制，Safari 不允許網頁控制音量。請使用手機側邊的實體音量鍵調整。
...
- 新增**瀏覽器收聽功能**：連上網頁的人可在手機或電腦直接聽到音樂。
- 伺服器新增 `/api/audio-stream` 音訊代理端點，將 YouTube 音訊串流轉發至瀏覽器。
- 前端新增「🎧 點擊收聽」按鈕、獨立瀏覽器音量控制、播放同步機制。
- 修正播放進度不會即時更新的問題（現在每 2 秒同步一次）。
- iOS 裝置自動偵測，以提示文字取代不支援的音量滑桿。
