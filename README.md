...
若 QR Code 要指向固定網址，可設定環境變數：

```bash
MUSIC_STATION_URL=http://192.168.1.100:8888
```
...
- 新增 YouTube 關鍵字搜尋：可直接輸入關鍵字，顯示候選結果後一鍵加入點歌。
- 前端新增搜尋結果列表（縮圖、標題、頻道、時長）與快速加入流程。
- 後端新增 `search_youtube` WebSocket 訊息處理，使用 `yt-dlp` 搜尋並回傳候選資料。
- `yt-dlp` 改為使用專案內 `.yt_tmp` 暫存資料夾，降低權限或暫存路徑問題造成的失敗率。
- 播放紀錄新增「再加入」按鈕，可把歷史歌曲快速加回待播佇列。

### 前端版本沒更新時（快取）

若瀏覽器還停在舊版前端，建議先：

1. 強制重整：`Ctrl + F5` 或 `Ctrl + Shift + R`
2. 使用無痕模式確認是否為快取問題
3. 清除該站點資料（DevTools -> Application -> Clear storage）
